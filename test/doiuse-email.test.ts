...
import { DoIUseEmail, doIUseEmail } from '../src/index';
...
describe('doIUseEmail() works', () => {
  ...
  test('getSupportedFeatures()', () => {
    const doIUse = new DoIUseEmail({ emailClients: ['*'] });
    const supportedFeatures = doIUse.getSupportedFeatures();
    ...
    expect(doIUse.getSupportedFeatures()).toMatchSnapshot();
  });
