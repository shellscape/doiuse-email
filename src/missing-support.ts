...
import { FeatureData } from '~/types/features.js';
import canIEmailData from '~/utils/data.js';

function filterStats(emailClients: string[], featureStats: FeatureData[]) {
	let result = {};
	for(const featureStat of featureStats) {
		const versions = featureStat.stats.

	}
}

const filterStats = (browsers, stats) =>
	Object.keys(stats).reduce(
		(result, browser) => {
			const versions = stats[browser];
			const feature = Object.keys(versions).reduce(
				(feat, version) => {
...
const lackingBrowsers = (browserStats) =>
	Object.keys(browserStats)
		.map((browser) =>
			formatBrowserName(browser, Object.keys(browserStats[browser]))
		)
		.join(', ');
