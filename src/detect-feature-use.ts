 ...
 */
function stripUrls(string: string) {
	// eslint-disable-next-line no-useless-escape
	return string.replace(/url\([^\)]*\)/g, 'url()');
}

/**
	@param string: string to search in (the haystack).
	@param searchFor: string or pattern to search for (the needle).
*/
function doesStringContain(string: string, searchFor: string | RegExp | ((needle: string) => boolean)) {
	string = stripUrls(string);
	if (searchFor instanceof RegExp) {
		return searchFor.test(string);
	} else if (typeof searchFor === 'function') {
		return searchFor(string);
	} else {
		return string.includes(searchFor)
	}
}

function isFoundIn(haystack: string) {
	haystack = stripUrls(haystack);
	return function find(needle: string | RegExp | ((str: string) => boolean)) {
